//map metodo : permette di ciclare tutti gli elementi di un array 
// e applicareli una modifica 
...
//filter : modifica gli elementi di un arry e restituisce
// un nuovo array con valori che hanno verificato la condizione
...
// filter non mischia i tipi per esempio non puoi interragire su due 
//array di tipo diversi(number[] e string[]) con filter come fai con map() 
...
// let valori:number[]=[1,2,3,4,5,10,12,15,20,25,33,30,40,90,93,130];
// let multiplo3:number[] = valori.filter(valore => valore %3 == 0);
// console.log(multiplo3);

// trova il primo numero pari in un array con find()
// find() si utilizza sempre con l'impostazione di 2 tipi speciale
// sia any oppure undefined pe prevenire un bug in caso 
// di non correspondenza o non verifica della condizione impostata
let valori: number[] = [
  1, 2, 3, 4, 10, 12, 15, 20, 25, 33, 30, 40, 90, 93, 130,
];
let paro: number | any = valori.find((num) => num % 2 == 0);
console.log(paro);
// utilizza find() per trovare la stringa con 6 caratteri in un array
let parole: string[] = ["ciao", "hello", "venizia", "ricco", "hojn", "isernia"];
let sei: string | any = parole.find((str) => str.length > 6);
console.log(sei);
//trova il primo valore maggiore di 20
let maggiore20: number | any = valori.find((num) => num > 20);
console.log(maggiore20);
// trovo il primo moltiplo di 5
let moltiplo5: number | undefined = valori.find((val) => val % 5 == 0);
console.log(moltiplo5);
